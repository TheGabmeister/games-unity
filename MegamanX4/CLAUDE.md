...
**Generators are scaffolding, not live config.** Once a prefab's tuning has settled in the editor (post-playtest), delete its generator method — regenerating would overwrite hand-tuned values. Keep methods only for enemies still in the "just spawned, tuning later" phase. The shared `EnemyGeneratorCore.cs` stays regardless.
...
6. **Contact-damage enemies must be `isTrigger: true`.** The player's Rigidbody2D is Kinematic and does its own swept-cast movement, ignoring the physics matrix for body resolution. Kinematic-vs-Kinematic `OnCollisionEnter2D` only fires when both bodies have `UseFullKinematicContacts` enabled — which nothing in this project does. So a non-trigger enemy collider silently fails to damage the player: the cast skips it, and collision callbacks never fire. `OnTriggerEnter2D` *does* fire across Kinematic pairs regardless, so `isTrigger = true` is the only path that lands contact damage. The exception is enemies that flip to Dynamic mid-behavior (Spike Marl post-drop).
7. **Physics-query scripts belong in `FixedUpdate`, not `Update`.** On play-mode entry, Physics2D doesn't sync collider transforms until the first `Simulate()` call (driven by `FixedUpdate`). A script that casts rays or queries overlaps from `Update` can see an empty world for the first few frames — enemies with gravity in `Update` will fall through the ground before physics catches up. `Gravity.cs` was moved to `FixedUpdate` for this reason.
