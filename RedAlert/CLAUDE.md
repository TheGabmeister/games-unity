...
- **Shared**: DisplayName, Sprite, DirectionSprites, Icon, Category, Faction, Cost, MaxHP, Armor, SightRange, Prefab
...
### Unit Sprite Sheets & Facing
Unit sprites are 8-direction sprite sheets: 512×64 px for vehicles/naval/aircraft (1 row), 512×256 px for infantry (4 rows: idle, walk1, walk2, fire). Directions: N, NE, E, SE, S, SW, W, NW. Generated by `Tools/generate_unit_sheets.py` (Python), exported via `Tools/export_sprites.sh` (Inkscape). Imported as Multiple sprite mode with 64×64 grid slicing by `GeneratePhase7Data.ImportSpriteSheets()`. The south-facing idle sprite is stored as `UnitData.Sprite`; the 8 idle-row sprites are stored in `UnitData.DirectionSprites`.

`FacingRenderer` component subscribes to `Mover.OnDirectionChanged` (fired when a unit starts moving toward a new cell). It picks the closest of 8 direction sprites via dot product and sets the SpriteRenderer. Mover has no knowledge of FacingRenderer — communication is event-only.

`LocomotionType.Fly` passes over all terrain at full speed. `TerrainMovement.SpeedTable` has 5 columns (Foot, Tracked, Wheeled, Float, Fly).
...
- **Sprites**: SVG source files in `Tools/sprites/` (subdirs: terrain, units, buildings, overlays, ui) exported to PNG via Inkscape. Unit sprites are 8-direction sheets (512×64 or 512×256 for infantry); see "Unit Sprite Sheets & Facing" above. Building sprites are `FootprintX × 64` by `FootprintY × 64` px (e.g., 3×3 CY = 192×192). The export script `Tools/export_sprites.sh` reads SVG native dimensions for units and has a `get_building_size` function for buildings.
  - Inkscape path: `"/c/Program Files/Inkscape/bin/inkscape.exe"`
  - Unit sheet generation: `python Tools/generate_unit_sheets.py` → `bash Tools/export_sprites.sh`
  ...
  - Unit sheet import: `SpriteImportMode.Multiple`, 64×64 grid slicing, handled by `GeneratePhase7Data.ImportSpriteSheets()`.
