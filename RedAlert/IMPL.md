...
- Data-driven minimap: `Texture2D` rendered from grid data (terrain colors, unit dots by player color, shroud black). Updates every 6 frames. `RawImage` in sidebar.
- Minimap requires Radar Dome + power. Shows "RADAR OFFLINE" overlay when unavailable.
- Click or drag on minimap jumps camera to that world position.
- CursorManager: context-sensitive hardware cursors (Select, Move, Attack, Harvest, NoGo, Sell, Repair). 32×32 PNGs generated by `GenerateCursors`.

**Testable**: Minimap reflects game state. Click minimap to jump camera. Radar Dome destroyed = minimap offline. Cursor changes when hovering enemies, ore, ground.
